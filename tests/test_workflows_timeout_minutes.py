...
stall during `pip install`, infinite loop in a chunker test, stuck
...
the next hop for `chunking-strategies-lab` per issue #41. Policy band
[1, 30] matches; every job here lands at 15 since none of the current
workloads (pure-python chunker tests, ruff, memory-file existence
checks) approach that ceiling.

Spec / origin: this repo's #41.
"""
...
# (e.g., a future late-chunking benchmark suite that hits embedding APIs).
...
def test_job_timeout_in_policy_band(workflow: str, job_id: str, body: dict[str, Any]) -> None:
    ...
    assert MIN_TIMEOUT_MINUTES <= timeout <= MAX_TIMEOUT_MINUTES, (
        ...
        f"late-chunking benchmark job), bump MAX_TIMEOUT_MINUTES with a "
